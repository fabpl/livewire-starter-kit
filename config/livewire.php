...
return [
    ...
    */
    ...
    /*
     * @note The `pages` entry the package ships with is removed rather than repointed. It binds
     * `resources/views/pages` to the view-based resolver, and `view_path` below binds the same
     * directory to the class-based one — one path with two resolvers, which resolves through
     * whichever is asked first and fails on the other. ADR-0003 chose the class-based idiom, so
     * this is the entry that goes. `layouts` stays: `component_layout` resolves through it.
     */
    ...
    */
    ...
    /*
     * @note The views directory itself rather than its `livewire` subdirectory, which is what
     * makes a component's view mirror its class path from the root and lets a Page carry no
     * `render` method: `App\Livewire\Pages\Home` renders `resources/views/pages/home.blade.php`.
     * The hazard it introduces — a component directly under the class namespace writing its view
     * among the ordinary views — is held by a test in `ArchitectureTest`, not by this note.
     */
