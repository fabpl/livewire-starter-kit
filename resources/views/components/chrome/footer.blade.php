{{-- The footer, composed by the Page for the same reason the bar is. Two facts and nothing else:
the licence the kit is released under, and where to read it. Both are checkable against this
repository — `composer.json` declares the licence, and the address is the one the installation
command in the hero names.

The repository link is an ordinary anchor rather than the button Primitive, and that is not an
oversight. The bar's link is a control in a row of controls and reads as one; a link in a line of
...
<footer class="border-t border-border">
    <div class="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-x-6 gap-y-2 px-6 py-8 text-sm text-muted-foreground">
